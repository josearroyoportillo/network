...
from app import app
from network_data import default_elements, temp_node_dict, \
    temp_edge_dict, add_edge_dict, add_node_dict, df

# df adjusted for table on app
df_table = df.copy()
df_table = df_table.rename(columns={'source': 'parent', 'target': 'node',
                                    'Einfluss/Wichtigkeit': 'Wichtigkeit'})
df_table = df_table.drop(['beeinflusst', 'beeinflusst_neg', 'node_class'], axis=1)


# load extra layouts
cyto.load_extra_layouts()

network_dataframe = html.Div([
    html.Div(id='table-content', children=[]),
    html.Button('Reset', id='reset_button_table', n_clicks=0),
    html.Div([
        cyto.Cytoscape(
...
@app.callback(
    Output('cytoscape_table', 'elements'),
    Input('cytoscape_table', 'tapNodeData'),
    Input('reset_button_table', 'n_clicks'),
    State('cytoscape_table', 'elements'))
...
@app.callback(
    ...
    Input('cytoscape_table', 'elements'))
def generate_table(elements):
    # each node is recorded into the list
    elms = [element.get('data').get('id') for element in elements]
    # table layout is defined and returned with the data from list
    table_data = df_table.loc[df_table['node'].isin(elms)].to_dict('records')
